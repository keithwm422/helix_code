...
// declare variables used for sampling the distribution
  ...
  double delta_R_over_R=0.012;  // minimum for He simulations (plot on helix proposal from Jim may be helpful).
  double delta_beta_over_beta=0.0002; //(minimum from RICH) // highest and lowest beta are 0.98 (negative or positive). best resolution is 2x10^-4. Gamma in detectable range is [1,5].
// quad_errors is the square root of the sum of the squares of the the resolutions of beta (with gamma factor) and R. 
  double quad_errors=TMath::Sqrt(pow(delta_R_over_R,2)+(pow(delta_beta_over_beta,2)*pow(,4)));
  double delta_m=mass*quad_errors;
// set delta_m by quadrature errors of rigidity and beta. before I picked a delta_m/m of 0.025 (m=10 or 9). Now I pick a deltaR/R and deltabeta/beta that gives me delta_m/m
